...
@section('title_postfix', ' - Detalhes da Atividade')
...
@section('content')
    ...
    <div class="card card-outline card-info">
        ...
        <div class="card-body pad">
            ...
            <p><strong>Data de Fim: </strong>{{ $data->end }}
            </p>
            <p><strong>Detalhes: </strong>
            <p>{!! $data->description !!}</p>

            <p><strong>Mostrar na agenda? {{ ($data->schedule == 1 ) ? ('Sim') : ('Não') }} </strong>
            <p><strong>Finalizado? {{ (!empty($data->finish)) ? ('Sim') : ('Não') }} </strong>

            @if (!empty($data->file))
                <p><a href="{{ route('events.fileDownload', $data->id) }}">Anexo</a></p>
            @endif
    ...
    @can('delete_phase')
        {!! Form::model($data, ['route' => ['events.destroy', $data->id], 'class' => 'form', 'method' => 'delete', 'id' => 'formDelete']) !!}
