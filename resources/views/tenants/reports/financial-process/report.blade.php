...
@section("body")
    <div id="corpo">
        ...
        @forelse($datas as $data)
            <div style="border: 1px solid #333; padding: 5px;margin-bottom:5px;">{{$data->name}}</div>
            @php
                $total = 0;
                $quant = 0;
            @endphp
            @forelse($data->processes as $process)
                ...
                @endphp
                <table>
                    @if(count($process->financials) > 0)
                        <tr>
                            <td colspan="5" style="padding-left:5px;padding-top:5px;border-bottom:0.01mm solid #333">
                                <strong>Processo: </strong> {{$process->number_process}}</td>
                        </tr>
                        <tr>
                            <td style="padding-left:5px;width:10%"><strong>Data</strong></td>
                            <td style="text-align:left"><strong>Documento</strong></td>
                            <td style="text-align:left"><strong>Vencimento</strong></td>
                        ...
                        @forelse($process->financials as $financial)
                            ...
                            <tr>
                                ...
                                <td style="text-align:left;">{{$financial->document }}</td>
                                <td style="text-align:left;">{{$financial->due_date_br}}</td>
                        ...
                        @endforelse
                        <tr>
                            <td colspan="4" style="text-align:right;margin-right:5"><strong>SubTotal:</strong></td>
                            <td colspan="1" style="text-align:right;">
                                <strong>{{App\Helpers\Helper::formatDecimal($credito - $debito,2)}}</strong></td>
                        </tr>
                        @php
            ...
            <table style="margin-top: 10px; margin-bottom:10px">
                <tr>
                    <td style="text-align:right;"><strong>Quantidade de processos {{$quant}} no valor
                            Total: {{App\Helpers\Helper::formatDecimal($total, 2)}}</strong></td>
                </tr>
            </table>
            @php
                $total_global += $total;
